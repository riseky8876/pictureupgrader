...
PipeLine::PipeLine()
    : codeformer_(new CodeFormer),
      real_esrgan_(new RealESRGAN),
      face_detector_(new Face) {}
...
static void paste_faces_to_input_image(const cv::Mat& restored_face,
    ...
    int w_edge = std::max(1, (int)(std::sqrt(total_face_area) / 20));
    ...
    // FIX: blur_size must be >= 1 and odd for GaussianBlur
    ...
    cv::GaussianBlur(inv_mask_center, inv_soft_mask,
                     cv::Size(blur_size, blur_size), 0, 0, 4);
...
int PipeLine::CreatePipeLine(PipelineConfig_t& pipeline_config) {
    ...
    if (codeformer_->Load(pipeline_config_.model_path) < 0) return -1;
    if (real_esrgan_->Load(pipeline_config_.model_path) < 0) return -1;
    if (face_detector_->Load(pipeline_config_.model_path) < 0) return -1;
    return 0;
}
...
int PipeLine::Apply(const cv::Mat& input_img, cv::Mat& output_img) {
    ...
    if (pipeline_config_.bg_upsample) {
        ...
        cv::resize(input_img, bg_upsample,
                   cv::Size(input_img.cols * 2, input_img.rows * 2), 0, 0, 1);
    }

    // FIX: allocate PipeResult_t on HEAP not stack — struct with 20 cv::Mat members
    // is too large for the default Android thread stack (1MB)
    auto pipe_result = std::make_unique<PipeResult_t>();
    pipe_result->face_count = 0;

    face_detector_->Process(input_img, (void*)pipe_result.get());

    for (int i = 0; i < pipe_result->face_count; ++i) {
        codeformer_->Process(pipe_result->object[i].trans_img,
                             pipe_result->codeformer_result[i]);
        paste_faces_to_input_image(pipe_result->codeformer_result[i].restored_face,
